...
TAXI_CONFIGS = {
    "fhv": {
        "schedule": "0 11 1 * *",
        "schema_sql": f"""
            CREATE TABLE IF NOT EXISTS `{{GCP_PROJECT_ID}}.{{GCP_DATASET}}.fhv_tripdata`(
            ...
            CREATE OR REPLACE EXTERNAL TABLE `{{GCP_PROJECT_ID}}.{{GCP_DATASET}}.fhv_tripdata_ext`(
                ...
                drop_off_datetime TIMESTAMP OPTIONS (description = 'The date and time when the trip ended'),
                PULocationID STRING OPTIONS (description = 'TLC Taxi Zone in which the trip started'),
                DOLocationID STRING OPTIONS (description = 'TLC Taxi Zone in which the trip ended'),
                SR_Flag STRING OPTIONS (description = 'Indicates if trip was part of a shared ride chain'),
                Affiliated_base_number STRING OPTIONS (description = 'The TLC base license number affiliated with the vehicle')
            )
            OPTIONS (
                format = 'CSV',
                uris = ['{{gcs_table}}'],
        ...
        "transform_n_merge_sql": f"""
            CREATE OR REPLACE TABLE `{{GCP_PROJECT_ID}}.{{GCP_DATASET}}.fhv_tripdata_stg` AS
            SELECT 
                MD5(CONCAT(
                    COALESCE(CAST(dispatching_base_num AS STRING), ""),
                    COALESCE(CAST(pickup_datetime AS STRING), ""),
                    COALESCE(CAST(drop_off_datetime AS STRING), ""),
                ...
                SAFE_CAST(SR_Flag AS STRING) AS SR_Flag,
                SAFE_CAST(Affiliated_base_number AS STRING) AS Affiliated_base_number
            FROM `{{GCP_PROJECT_ID}}.{{GCP_DATASET}}.fhv_tripdata_ext`;

            MERGE INTO `{{GCP_PROJECT_ID}}.{{GCP_DATASET}}.fhv_tripdata` T
            USING `{{GCP_PROJECT_ID}}.{{GCP_DATASET}}.fhv_tripdata_stg` S ON T.unique_row_id = S.unique_row_id
            WHEN NOT MATCHED THEN
                INSERT (unique_row_id, filename, dispatching_base_num, pickup_datetime, drop_off_datetime, PULocationID, DOLocationID, SR_Flag, Affiliated_base_number)
                VALUES (S.unique_row_id, S.filename, S.dispatching_base_num, S.pickup_datetime, S.drop_off_datetime, S.PULocationID, S.DOLocationID, S.SR_Flag, S.Affiliated_base_number);
                
            DROP TABLE IF EXISTS `{{GCP_PROJECT_ID}}.{{GCP_DATASET}}.fhv_tripdata_stg`;
        """
    },
    "green": {
        "schedule": "0 9 1 * *",
    ...
    def gcp_taxi_scheduled_pipeline():
        ...
        def set_label(**context) -> list[dict]:
            ...
            # Automated scheduled run fallback
                ...
                ym_list = logical_date.strftime("%Y-%m")
